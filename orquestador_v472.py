#!/usr/bin/env python3
"""
UTSF-MFC v4.7.2 — Orquestador Multi-IA Autónomo (Corregido)
Correcciones: endpoints y modelos actualizados, permisos de escritura
"""
...
class OrquestadorUTSF:
    def __init__(self):
        ...
        # Configuración de endpoints (v4.7.2 - corregida)
        self.config = {
            "groq": {
                "url": "https://api.groq.com/openai/v1/chat/completions",
                "key": os.getenv("GROQ_API_KEY"),
                "modelo": "llama-3.3-70b-versatile",  # ✅ Cambiado a modelo activo
                "peso": 1.0,
            ...
            "cohere": {
                "url": "https://api.cohere.ai/v1/generate",  # ✅ Endpoint nativo (no OpenAI-compatible)
            ...
            "gemini": {
                "url": "https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash-latest:generateContent",  # ✅ Usar -latest
            ...
            "qwen": {
                ...
                "modelo": "qwen-turbo",  # ✅ Cambiado a turbo (más probable)
                "peso": 1.0,
            ...
            "openrouter": {
                ...
                "modelo": "google/gemma-2-9b-it:free",  # ✅ Alternativa free verificada
                "peso": 0.8,
    ...
    async def consultar_cohere(self, session: aiohttp.ClientSession) -> RespuestaIA:
        ...
        # Cohere nativo: endpoint /v1/generate con payload 'prompt'
        data, error, status, latencia = await self._llamar_api(
            ia_id, session,
            cfg["url"],
            {
        ...
        try:
            contenido = data["generations"][0]["text"]
            log(f"✅ {ia_id}: Respuesta recibida ({len(contenido)} chars)")
    ...
    async def ejecutar_ciclo(self):
        ...
        log(f"\n{'='*60}")
        log(f"🚀 UTSF-MFC v4.7.2 | Ciclo: {self.timestamp}")
    ...
    def guardar_resultados(self, respuestas: List[RespuestaIA], consenso: Dict):
        ...
        consenso["version_orquestador"] = "v4.7.2"
    ...
    async def notificar_telegram(self, consenso: Dict):
        ...
        mensaje = f"""{estado_emoji} UTSF-MFC v4.7.2 — Ciclo completado
...
async def main():
    ...
    log(f"\n{'='*60}")
    log("  UTSF-MFC v4.7.2 — ORQUESTADOR MULTI-IA AUTÓNOMO")
