...
# ═══════════════════════════════════════════════════════════════
# UTSF-MFC v8.8.8 – Orquestador Multi-IA v4.6.1 (Corregido)
# 6 IAs autonomas + 6 manuales + 4 reserva + 1 candidata
# Fixes: cryptography para Gemini, Cohere diagnostico corregido,
...
def check_env():
    ...
    print(f"🤖 Autonomas ({disponibles}/6): {', '.join([k for k,v in ias.items() if v]) or 'Ninguna'}")
...
# ─── LLAMADAS A APIs CON MANEJO DE EXCEPCIONES ───
...
def llamar_gemini(prompt, location="us-central1"):
    """Llama a Gemini via Vertex AI con autenticacion JWT de cuenta de servicio."""
    ...
    try:
        ...
        # ✅ cryptography instalada en workflow permite RS256
...
def llamar_qwen(prompt, modelo="qwen-max", max_tokens=2000):
    ...
    # ✅ Fallback multinivel: si qwen-max falla, prueba plus, luego turbo
...
# ─── COHERE MULTILINGUE (SIN CAMBIOS – v1/embed sigue activo) ───
def analisis_semantico_cohere(texto_estado):
    """Analisis semantico usando la API v1 de Cohere (embeddings)."""
    ...
    # ✅ Endpoint v1/embed sigue activo y funcional
    ...
    try:
        ...
        if r.status_code != 200:
            print(f"❌ Cohere: HTTP {r.status_code}")
            return None
...
def main():
    ...
    # ═══════════════════════════════════════════════════════════════
    # FASE 0: DIAGNOSTICO DE CONECTIVIDAD
    # ═══════════════════════════════════════════════════════════════
    print("\n" + "=" * 60)
    ...
    # Cohere – ✅ CORREGIDO: Usar endpoint v1/embed (vigente) en lugar de v1/generate (obsoleto)
    if COHERE_API_KEY:
        st, msg, t = diagnostico_api(
            "Cohere", COHERE_API_KEY,
            "https://api.cohere.com/v1/embed",  # ✅ Endpoint correcto
    ...
    respuestas['validador'], respuestas['validador_ia'] = llamar_con_fallback(
        ...
        ["deepseek-reasoner", "qwen", "groq"],  # Claude eliminado
        2000, 0.3
    )
    ...
    # ═══════════════════════════════════════════════════════════════
    # FASE 4: ANALISIS DE CORPUS (unificado con fallback)
    # ═══════════════════════════════════════════════════════════════
    print("\n" + "="*60)
