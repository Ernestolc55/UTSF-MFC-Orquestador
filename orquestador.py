...
# ═══════════════════════════════════════════════════════════════
# UTSF-MFC v8.8.8 – Orquestador Multi-IA v4.6.0 (Vertex AI)
# 6 IAs autónomas + 6 manuales + 4 reserva + 1 candidata
# ═══════════════════════════════════════════════════════════════
...
def check_env():
    ...
    print(f"🔧 UTSF-MFC v4.6.0 | Ciclo {CICLO_ID}")
...
def parser_claude(json_resp):
    return json_resp["content"][0]["text"]
...
def llamar_gemini(prompt, location="us-central1"):
    ...
    if not GEMINI_PRIVATE_KEY or not GEMINI_CLIENT_EMAIL or not GOOGLE_PROJECT_ID:
        print("❌ Gemini: Faltan credenciales de cuenta de servicio o PROJECT_ID")
        return None
...
def llamar_qwen(prompt, modelo="qwen-max", max_tokens=2000):
    ...
    return llamar_api_generica("Qwen", url, headers, data, 60, parser_qwen)[0]
